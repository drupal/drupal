...
class MimeTypeGuesserTest extends UnitTestCase {
   ...
   */
  public function testSymfonyMimeTypesGuesserRegistration() {
    // Force new instance so the previous test does not affect this one.
    $symfony_mime_types = new SymfonyMimeTypes();
    SymfonyMimeTypes::setDefault($symfony_mime_types);

    // Make the guessers property accessible on Symfony's MimeTypes.
    $symfony_mime_types = SymfonyMimeTypes::getDefault();
    // Test that the Drupal mime type guess is not being used before the
    // override method is called. It is possible that the test environment does
    // not support the default guessers.
    $reflected_mime_types = new \ReflectionProperty($symfony_mime_types, 'guessers');
    $reflected_mime_types->setAccessible(TRUE);
    $guessers = $reflected_mime_types->getValue($symfony_mime_types);
    if (count($guessers)) {
      $this->assertNotInstanceOf('Drupal\Core\File\MimeType\MimeTypeGuesser', $guessers[0]);
    }
    $container = new ContainerBuilder();
