...
class MenuLinkDefaultIntegrationTest extends KernelTestBase {
  ...
  public function testMoveToRoot() {
    ...
    $this->assertEqual('menu_test.parent', $menu_link['parent']);
    $this->assertEqual('test', $menu_link['menu_name']);

    $tree = \Drupal::menuTree()->load('test', new MenuTreeParameters());
    $this->assertCount(1, $tree);
    $this->assertEqual('menu_test.parent', $tree['menu_test.parent']->link->getPluginId());
    $this->assertEqual('menu_test.child', $tree['menu_test.parent']->subtree['menu_test.child']->link->getPluginId());
    ...
    $this->assertEqual('menu_test.parent', $tree['menu_test.parent']->link->getPluginId());
    $this->assertEqual('menu_test.child', $tree['menu_test.child']->link->getPluginId());
