...
class StatementTest extends DatabaseTestBase {
  ...
  public function testRepeatedInsertStatementReuse() {
    ...
    $this->assertSame(30, (int) $this->connection->query('SELECT [age] FROM {test} WHERE [name] = :name', [':name' => 'Larry'])->fetchField());
    $this->assertSame(31, (int) $this->connection->query('SELECT [age] FROM {test} WHERE [name] = :name', [':name' => 'Curly'])->fetchField());
  }
