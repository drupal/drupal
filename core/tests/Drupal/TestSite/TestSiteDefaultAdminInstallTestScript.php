 ...
 */
class TestSiteDefaultAdminInstallTestScript implements TestSetupInterface {
  ...
  public function setup(): void {
    ...
    // Install Default Admin and set it as the default theme.
    $theme_installer = \Drupal::service('theme_installer');
    assert($theme_installer instanceof ThemeInstallerInterface);
    $theme_installer->install(['default_admin'], TRUE);
    $system_theme_config = \Drupal::configFactory()->getEditable('system.theme');
    $system_theme_config->set('default', 'default_admin')->save();
