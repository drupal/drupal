((Drupal, once) => {
  Drupal.behaviors.adminMediaLibrary = {
    attach: function attach() {
      Drupal.adminMediaLibrary.init();
    },
  };

  Drupal.adminMediaLibrary = {
    init: function () {
      once(
        ...
        if (el.querySelectorAll('.js-media-library-item').length) {
          ...
          selectAll.innerHTML =
            Drupal.theme('checkbox') + Drupal.t('Select all media');
          selectAll.children[0].addEventListener('click', (e) => {
            ...
            const announcement = currentTarget.checked
    ...
    itemSelect: () => {
      document
        .querySelectorAll(
          '.media-library-view .js-click-to-select-trigger, .media-library-view .media-library-item .form-checkbox',
        )
        .forEach((trigger) => {
          trigger.addEventListener('click', () => {
            const selectAll = document.querySelector(
              '.media-library-select-all .form-boolean',
            );
            const checkboxes = document.querySelectorAll(
              '.media-library-view .media-library-item .form-boolean',
            );
            const checkboxesChecked = document.querySelectorAll(
              '.media-library-view .media-library-item .form-boolean:checked',
            );
            ...
            if (
              ...
              checkboxes.length ===
              Array.from(checkboxes).filter((el) => el.checked === true).length
            ) {
              selectAll.checked = true;
              selectAll.dispatchEvent(new Event('change'));
            }

            Drupal.adminMediaLibrary.bulkOperations();
          });
        });
    },

    bulkOperations: () => {
      const bulkOperations = document.querySelector(
        '.media-library-view [data-drupal-selector*="edit-header"]',
      );
      const bulkOperationsStickyBar = document.querySelector(
        '.media-library-views-form__bulk_form',
      );

      if (
        bulkOperations &&
        document.querySelectorAll('.media-library-view .form-checkbox:checked')
          .length > 0
      ) {
