...
class ThemeHooks implements TrustedCallbackInterface {
  ...
  public function pageAttachmentsAlter(array &$page): void {
    $theme_path = $this->requestStack->getCurrentRequest()?->getBasePath() . '/' . $this->themeExtensionList->getPath('default_admin');
    ...
    $page['#attached']['library'][] = 'default_admin/init';

    // Attach breadcrumb styles.
    $page['#attached']['library'][] = 'default_admin/breadcrumb';

    // Attach accent library.
    $page['#attached']['library'][] = 'default_admin/accent';
  ...
  public function themeRegistryAlter(array &$theme_registry): void {
    ...
    foreach (['toolbar', 'menu__toolbar'] as $registry_item) {
      if (isset($theme_registry[$registry_item])) {
        $theme_registry[$registry_item]['path'] = 'core/themes/default_admin/templates/navigation';
      }
    }
  }
