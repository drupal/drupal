...
interface UpdateManagerInterface {
   ...
   * dev snapshot, etc) is handled later in
   * \Drupal\update\UpdateCalculator::processProjectInfo() since that logic is
   * only required when preparing the status report, not for fetching the
   ...
   *   \Drupal\update\UpdateManagerInterface::calculateProjectData() or
   *   \Drupal\update\UpdateManagerInterface::getProjects(), or an empty array
   ...
   *
   * @param bool $refresh
   *   (optional) Boolean to indicate if this method should refresh
   ...
   *
   * The results of this method are expensive to compute, especially on sites
   * with lots of modules or themes, since it involves a lot of comparisons and
   * other operations. Therefore, we store the results. However, since this is
   * not the data about available updates fetched from the network, it is ok to
   * invalidate it somewhat quickly. If we keep this data for very long, site
   * administrators are more likely to see incorrect results if they upgrade to
   * a newer version of a module or theme but do not visit certain pages that
