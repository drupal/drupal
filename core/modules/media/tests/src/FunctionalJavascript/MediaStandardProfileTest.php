...
class MediaStandardProfileTest extends MediaJavascriptTestBase {
  ...
  protected function audioTest(): void {
    ...
    // Here we expect to see only the linked filename. Assert the audio element
    // exists.
    $assert_session->elementsCount('css', '#block-default-admin-content audio', 1);

    // Assert the audio file is present inside the media element and that its
    // src attribute matches the audio file.
    $audio_element = $assert_session->elementExists('css', '#block-default-admin-content audio > source');
    ...
    // Again we expect to see only the linked filename. Assert only one audio
    // field in the content region.
    $assert_session->elementsCount('css', '#block-default-admin-content audio', 1);
  ...
  protected function imageTest(): void {
    ...
    $assert_session->elementsCount('css', '#block-default-admin-content img', 1);
    ...
    $image_element = $assert_session->elementExists('css', '#block-default-admin-content img');
    /** @var \Drupal\Core\File\FileUrlGeneratorInterface $file_url_generator */
    $file_url_generator = \Drupal::service('file_url_generator');
    ...
    $assert_session->elementExists('css', '#block-default-admin-content div.visually-hidden');
    $assert_session->elementNotExists('css', '#block-default-admin-content img a');
  ...
  protected function documentTest(): void {
    ...
    $assert_session->elementsCount('css', '#block-default-admin-content span.file', 1);

    // Assert the file link is present in the media element and its text matches
    // the filename.
    $link_element = $assert_session->elementExists('css', '#block-default-admin-content span.file a');
  ...
  protected function remoteVideoTest(): void {
    ...
    // the content region.
    $assert_session->elementsCount('css', '#block-default-admin-content iframe', 1);

    // Assert the iframe is present in the media element and its src attribute
    // matches the URL and query parameters.
    $iframe_url = $assert_session->elementExists('css', '#block-default-admin-content iframe')->getAttribute('src');
  ...
  protected function videoTest(): void {
    ...
    // Here we expect to see only the linked filename. Assert only one element
    // in the content region.
    $assert_session->elementsCount('css', '#block-default-admin-content video', 1);

    // Assert the video element is present inside the media element and that its
    // src attribute matches the video file.
    $video_element = $assert_session->elementExists('css', '#block-default-admin-content video > source');
    /** @var \Drupal\Core\File\FileUrlGeneratorInterface $file_url_generator */
    $file_url_generator = \Drupal::service('file_url_generator');
