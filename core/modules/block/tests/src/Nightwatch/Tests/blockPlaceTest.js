module.exports = {
  ...
  'Test once on block filter': (browser) => {
    ...
    browser.expect.elements('[data-once="block-filter-text"]').count.to.equal(1);
    browser.executeAsync(
      ...
      [],
      (result) => {
        browser.assert.equal(result.value.length, 0, 'once returned no elements')
      }
    )
    browser.executeAsync(
      function (done) {
      ...
      [],
      (result) => {
        browser.assert.equal(result.value.length, 0, 'once returned no elements after second attachBehaviors')
      }
    )
  },
  'Test block highlight on placement': (browser) => {
    ...
    browser.executeAsync(
      ...
      [],
      (result) => browser.assert.equal(result.value.windowOffset, result.value.elementOffset, 'window was scrolled to the placed block')
    )
  }
};
