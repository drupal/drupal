...
class MigrateCommentTest extends MigrateDrupal6TestBase {
  ...
  public function testMigration() {
    ...
    $this->assertSame(1, (int) $comment->getCommentedEntityId());
    ...
    $this->assertSame(3, (int) $comment->pid->target_id);
    ...
    $this->assertSame(21, (int) $comment->getCommentedEntityId());
    $this->assertSame('node', $comment->getCommentedEntityTypeId());
    $this->assertSame('en', $comment->language()->getId());

    $node = $comment->getCommentedEntity();
    $this->assertInstanceOf(NodeInterface::class, $node);
    $this->assertSame(21, (int) $node->id());
