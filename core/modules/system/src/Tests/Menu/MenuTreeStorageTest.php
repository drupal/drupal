...
class MenuTreeStorageTest extends KernelTestBase {
  ...
  public function testLoadTree() {
    ...
    $result = $this->treeStorage->loadTree('tools');
    $this->assertEqual(count($result['test1']['below']), 1);
    $this->assertEqual(count($result['test1']['below']['test2']['below']), 1);
    $this->assertEqual(count($result['test1']['below']['test2']['below']['test3']['below']), 0);
    $this->assertEqual(count($result['test4']['below']), 1);
    $this->assertEqual(count($result['test4']['below']['test5']['below']), 0);

    $result = $this->treeStorage->loadTree('tools', array('active_trail' => array('test4', 'test5')));
    $this->assertEqual(count($result['test1']['below']), 1);
    $this->assertFalse($result['test1']['in_active_trail']);
    $this->assertEqual(count($result['test1']['below']['test2']['below']), 1);
    $this->assertFalse($result['test1']['below']['test2']['in_active_trail']);
    $this->assertEqual(count($result['test1']['below']['test2']['below']['test3']['below']), 0);
    $this->assertFalse($result['test1']['below']['test2']['below']['test3']['in_active_trail']);
    $this->assertEqual(count($result['test4']['below']), 1);
    $this->assertTrue($result['test4']['in_active_trail']);
    $this->assertEqual(count($result['test4']['below']['test5']['below']), 0);
    $this->assertTrue($result['test4']['below']['test5']['in_active_trail']);

    $result = $this->treeStorage->loadTree('tools', array('active_trail' => array('test4', 'test5'), 'only_active_trail' => TRUE));
    $this->assertTrue(empty($result['test1']));
    $this->assertEqual(count($result['test4']['below']), 1);
    $this->assertTrue($result['test4']['in_active_trail']);
    $this->assertEqual(count($result['test4']['below']['test5']['below']), 0);
    $this->assertTrue($result['test4']['below']['test5']['in_active_trail']);
