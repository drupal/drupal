...
class DateFormatsMachineNameTest extends BrowserTestBase {
  ...
  public function testDateFormatsMachineNameAllowedValues() {
    ...
    $this->drupalPostForm('admin/config/regional/date-time/formats/add', $edit, 'Add format');
    $this->assertText('The machine-readable name must be unique, and can only contain lowercase letters, numbers, and underscores. Additionally, it can not be the reserved word "custom".', 'It is not possible to create a date format with the machine name "custom".');
    ...
    $this->drupalPostForm('admin/config/regional/date-time/formats/add', $edit, 'Add format');
    $this->assertText('The machine-readable name is already in use. It must be unique.', 'It is not possible to create a date format with the machine name "fallback". It is a built-in format that already exists.');
    ...
    $this->drupalPostForm('admin/config/regional/date-time/formats/add', $edit, 'Add format');
    $this->assertText('Custom date format added.', 'It is possible to create a date format with a new machine name.');
    ...
    $this->drupalPostForm('admin/config/regional/date-time/formats/add', $edit, 'Add format');
    $this->assertText('The machine-readable name is already in use. It must be unique.', 'It is not possible to create a new date format with an existing machine name.');
  }
