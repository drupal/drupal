...
class SessionHttpsTest extends BrowserTestBase {
  ...
  protected function loginHttps(AccountInterface $account): void {
    ...
    // cookie with the secure attribute set. Guzzle 8 does not store a secure
    // cookie received over the insecure HTTPS mock connection, so fall back to
    // parsing the Set-Cookie header to confirm the cookie was issued with the
