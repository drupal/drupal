  ...
  protected function doTestTranslationOverview() {
    ...
    foreach ($this->langcodes as $langcode) {
      if ($entity->hasTranslation($langcode)) {
        ...
        $this->assertEquals($entity->getTranslation($langcode)->label() ?? $entity->id(), $elements[0]->getText(), new FormattableMarkup('Label correctly shown for %language translation.', ['%language' => $langcode]));
