...
// $Id: dev_install.inc.php 4783 2010-12-24 09:32:25Z svowl $
...
function install_tasks($install_state) {
  ...
  // [DEV_CODE]: Skip some litecommerce_install_tasks_alter()
  $tasks['litecommerce_license_form']['run'] = INSTALL_TASK_SKIP;
  $tasks['litecommerce_setup_form']['run'] = INSTALL_TASK_SKIP;
  $tasks['litecommerce_software_install']['run'] = INSTALL_TASK_SKIP;
  // [/DEV_CODE]
...
function install_redirect_url($install_state) {
  return 'dev_install.php?' . drupal_http_build_query($install_state['parameters']);
}
...
/profiles/' . $profilename . '/translations/
          ...
          $output .= '<ul>';
          $output .= '<li><a href="dev_install.php?profile=' . $profilename . '">Reload the language selection page after adding translations</a></li>';
          $output .= '<li><a href="dev_install.php?profile=' . $profilename . '&amp;locale=en">Continue installation in English</a></li>';
          $output .= '</ul>';
        }
        else {
          include_once DRUPAL_ROOT . '/includes/form.inc';
...
function install_select_locale_form($form, &$form_state, $locales, $profilename) {
  ...
  if (count($locales) == 1) {
    $form['help'] = array(
      '#markup' => '<p><a href="dev_install.php?profile=' . $profilename . '&amp;localize=true">' . st('Learn how to install Drupal in other languages') . '</a></p>',
    );
  }
  $form['actions'] = array('#type' => 'actions');
  $form['actions']['submit'] =  array(
...
function install_configure_form($form, &$form_state, &$install_state) {
  ...
  return dev_install_configure_form($return);
}
